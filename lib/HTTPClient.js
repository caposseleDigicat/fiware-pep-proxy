...
exports.sendData = function(port, options, data, res, callBackOK, callbackError) {
    ...
    callBackOK = callBackOK || function(status, resp, headers) {
        ...
        log.error("LOG Response:", status);
        logger.responseStatus = status;
        log.error("LOG Headers:", JSON.stringify(headers));
        logger.responseHeaders = JSON.stringify(headers);
        if(resp !== ""){
            log.error("LOG Body:", JSON.stringify(JSON.parse(resp)));
    ...
    log.error("LOG Method:", options.method);
    logger.method = options.method;
    log.error("LOG URL:", options.path);
    logger.url = options.path;
    log.error("LOG Request Headers:", options.headers);
...
//            case "cookie":
    ...
    if (data !== undefined) {
        ...
        else if (options.path !== "/v3/auth/tokens"){
            log.error("LOG Request body:", JSON.stringify(JSON.parse(data)));
