...
var RBAC = (function() {
    ...
    var check_permissions = function(auth_token, user_info, req, callback, callbackError) {
        ...
        switch(get_path(resource)){
            ...
            case QUERY:{
                ...
                if (check_type.type === undefined){
                    callbackError(401, 'User token not authorized: missing entity type');
                    return;
                }
                var permissions_required = parse_permissions_from_request(req);
                log.info('QUERY - Permissions required: ', permissions_required);
                for (roleIdx in roles){
                    //log.info('role:', roles[roleIdx])
                    var permissions_owned = parse_permissions_from_role(roles[roleIdx]);
                    log.info('Permissions owned:', permissions_owned);
            ...
            case ENTITY: {
                ...
                if(action === 'GET'){
                    ...
                    log.info('ENTITY - Permissions required: ', permissions_required);
                    for (roleIdx in roles){
                        //log.info('role:', roles[roleIdx])
                        var permissions_owned = parse_permissions_from_role(roles[roleIdx]);
                        log.info('Permissions owned:', permissions_owned);
            ...
            case SUBSCRIPTION: {
                if(action === 'GET' || action === 'DELETE'){
                    ...
                    log.info('SUBSCRIPTION - Permissions required: ', permissions_required);
                    var permissions_owned = [];
                    
                    for (roleIdx in roles){
                        //log.info('role:', roles[roleIdx])
                        permissions_owned[roleIdx] = parse_permissions_from_role(roles[roleIdx]);
                        log.info('Permissions owned:', permissions_owned[roleIdx]); 
                    }
    ...
    var parse_permissions_from_request = function (request) {
        ...
        //retrieve operation
        permissions.operation = request.method; // TO IMPROVE
        
        //retrieve entity type
        if (parsedQs.type !== undefined)
            permissions.type = parsedQs.type;
        
        //retrieve entity id
        if (parsedQs.id !== undefined)
            permissions.id = parsedQs.id;
        else if (parsedQs.idPattern !== undefined)
            permissions.id = parsedQs.idPattern;
        
        //retrieve attributes    
        if (parsedQs.attrs !== undefined)
            permissions.attrs = parsedQs.attrs;
