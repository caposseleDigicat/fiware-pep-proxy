...
var RBAC = (function() {
    ...
    var check_permissions = function(auth_token, user_info, req, callback, callbackError) {
        ...
        //Data_provider bypass
        for (var roleIdx in roles){
            if (roles[roleIdx] === "data_provider")  {
                isDataProvider = true;
            }
            if (roles[roleIdx] === "|GET|*||") {
                canReadAll = true;
            }
        }
        if (isDataProvider) {
        ...
        switch(get_path(resource)){
            ...
            case SUBSCRIPTION: {
                if(action === 'GET' || action === 'DELETE'){
                    ...
                    if (subscription_is_allowed(permissions_required, permissions_owned) || canReadAll) {
                        callback();
                        return;
                    }
                }
    ...
    var get_path = function(resource) {
        ...
        if (resource === 'v2' || resource === 'v2/' || resource.indexOf('v2/types') !== -1)
            ...
            return SUBSCRIPTION;
        else if (resource.indexOf("v2/entities") !== -1 && (temp[2] === '' || temp[2] === undefined))
            return QUERY;
